 ...
 * @LastEditTime: 2025-10-28 12:27:44
 * @FilePath: \Task\mycode\HW_fdcan.cpp
  ...
  if (hfdcan == &hfdcan1) {
        ...
        HAL_OK) // 获得接收到的数据头和数据
    {
      if (rx_header1.Identifier == 0x208) { // 帧头校验
        // 校验通过进行具体数据处理
        motor.realAngle = (int16_t)(((can1_rx_data[0] << 8) | can1_rx_data[1]) / 8192.0 * 360 - 180);
        motor.realSpeed = (int16_t)((can1_rx_data[2] << 8) | can1_rx_data[3]);
      }
    }
